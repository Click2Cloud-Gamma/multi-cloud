...
func (mover *CephS3Mover) ChangeStorageClass(objKey *string, newClass *string, loca *BackendInfo) error {
	...
	if err != nil {
		log.Errorf("[cephs3lifecycle] Change storage class of object[%s] to %s failed: %v.\n", objKey, newClass, err)
...
func (mover *CephS3Mover) DeleteIncompleteMultipartUpload(objKey, uploadId string, loc *LocationInfo) error {
	...
	if e == nil || e.Error() == DMERR_NoSuchUpload {
		log.Infof("[cephs3lifecycle] abort multipart upload[objkey:%s, uploadid:%s] successfully.\n", objKey, uploadId)
		return nil
	} else {
