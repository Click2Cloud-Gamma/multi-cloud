...
func (ad *CephAdapter) Put(ctx context.Context, stream io.Reader, object *pb.Object) (dscommon.PutResult, error) {
	bucket := ad.backend.BucketName
	objectId := object.BucketName + "/" + object.ObjectKey
	result := dscommon.PutResult{}
	userMd5 := dscommon.GetMd5FromCtx(ctx)
	size := object.Size
	log.Infof("put object[Ceph S3], objectId:%s, bucket:%s, size=%d, userMd5=%s\n", objectId, bucket, size, userMd5)
	...
	log.Infof("upload object[Ceph S3] start, objectId:%s\n", objectId)
	ret, err := uploader.Upload(input)
	if err != nil {
		log.Errorf("put object[Ceph S3] failed, objectId:%s, err:%v\n", objectId, err)
	...
	result.Written = size
	log.Infof("put object[Ceph S3] successfully, objectId:%s, UpdateTime is:%v\n", objectId, result.UpdateTime)
...
func (ad *CephAdapter) Get(ctx context.Context, object *pb.Object, start int64, end int64) (io.ReadCloser, error) {
	...
	log.Infof("get object[Ceph S3], objectId:%s, start = %d, end = %d\n", objectId, start, end)
	if start != 0 || end != 0 {
	...
	if err != nil {
		log.Errorf("get object[Ceph S3] failed, objectId:%s, err:%v", objectId, err)
		return nil, ErrGetFromBackendFailed
	}
	log.Infof("get object[Ceph S3] succeed, objectId:%s, ContentLength:%d\n", objectId, *result.ContentLength)
	return result.Body, nil
}
func (ad *CephAdapter) Delete(ctx context.Context, input *pb.DeleteObjectInput) error {
	bucket := ad.backend.BucketName
	objectId := input.Bucket + "/" + input.Key
	deleteInput := awss3.DeleteObjectInput{Bucket: &bucket, Key: &objectId}

	log.Infof("delete object[Ceph S3], objectId:%s.\n", objectId)
	svc := awss3.New(ad.session)
...
func (ad *CephAdapter) ChangeStorageClass(ctx context.Context, object *pb.Object, newClass *string) error {
	objectId := object.ObjectId
	log.Infof("change storage class[Ceph S3] of object[%s] to %s .\n", objectId, *newClass)
	svc := awss3.New(ad.session)
	...
	if err != nil {
		log.Errorf("change storage class[Ceph S3] of object[%s] to %s failed: %v.\n", objectId, *newClass, err)
		return ErrPutToBackendFailed
	}
	log.Infof("change storage class[Ceph S3] of object[%s] to %s succeed.\n", objectId, *newClass)
	return nil
}
func (ad *CephAdapter) InitMultipartUpload(ctx context.Context, object *pb.Object) (*pb.MultipartUpload, error) {
	...
	storClass, err := osdss3.GetNameFromTier(object.Tier, utils.OSTYPE_CEPH)
	if err != nil {
		log.Warnf("translate tier[%d] to ceph storage class failed, use default value.\n", object.Tier)
	...
	if err != nil {
		log.Fatalf("init multipart upload[Ceph S3] failed, err:%v\n", err)
		return nil, ErrBackendInitMultipartFailed
	} else {
		log.Infof("init multipart upload[Ceph S3] succeed, UploadId:%s\n", *res.UploadId)
...
func (ad *CephAdapter) UploadPart(ctx context.Context, stream io.Reader, multipartUpload *pb.MultipartUpload,
	...
	log.Infof("upload part[Ceph S3], input:%v\n", *upPartInput)
	...
	if err != nil {
		...
		log.Infof("upload object[Ceph S3], objectId:%s, part #%d succeed, ETag:%s\n", multipartUpload.ObjectId,
...
func (ad *CephAdapter) CompleteMultipartUpload(ctx context.Context, multipartUpload *pb.MultipartUpload,
	...
	bucket := ad.backend.BucketName
	log.Infof("complete multipart upload[Ceph S3], bucket:%s, objectId:%s.\n", bucket, multipartUpload.ObjectId)
	...
	if err != nil {
		log.Errorf("complete multipart upload[Ceph S3] failed, err:%v\n", err)
	...
	log.Infof("complete multipart upload[Ceph S3] successfully, resp:%v\n", resp)
	return result, nil
}
func (ad *CephAdapter) AbortMultipartUpload(ctx context.Context, multipartUpload *pb.MultipartUpload) error {
	bucket := ad.backend.BucketName
	log.Infof("abort multipart upload[Ceph S3], bucket:%s, objectId:%s.\n", bucket, multipartUpload.ObjectId)
	...
	if err != nil {
		log.Errorf("abort multipart upload[Ceph S3] failed, err:%v\n", err)
		return ErrBackendAbortMultipartFailed
	}
	log.Infof("complete multipart upload[Ceph S3] successfully, rsp:%v\n", rsp)
	return nil
}
